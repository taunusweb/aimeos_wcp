        ...
        <?php if( $item->getStatus() > 0 ) : ?>
            ...
            <?php endif; ?>
                <?php if( count( $item->getChildren() ) > 0 ) : ?>
                    <?php $values = array( 'nodes' => $item->getChildren(), 'path' => $path, 'params' => $params, 'level' => $level + 1 , 'currentLevel' =>$this->get( 'currentLevel', 0 )); ?>
                    <?= $this->partial( $this->config( 'client/html/catalog/filter/partials/tree', 'catalog/filter/tree-partial-wcp' ), $values ); ?>
                <?php endif; ?>
            <?php if ($this->get( 'currentLevel', 0 ) < $item->getLevel()) : ?>
			</div>
            <?php endif; ?>
